...
function EditProgram(props) {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setInputs(res.data.exercises);
        setTitle(res.data.title);
        setLink(res.data.background);
      })
      .catch((err) => {
  ...
  function submitProgram() {
    ...
    axios
      .put(`http://localhost:3001/programs/edit/${params.id}`, data)
      .then((res) => {
        navigate("/admin");
      })
      .catch((err) => {
    ...
    <div className="min-h-full flex flex-col p-2 pt-4 md:p-10 lg:p-10 xl:p-10">
      <div className="container  max-w-lg m-auto  flex flex-1 flex-col items-center justify-center px-20 ">
        <div className="bg-white flex flex-col px-6 py-8 rounded shadow-md text-black cardWidth  ">
          ...
          >
            <h1 className="mb-8 text-3xl text-center">Edit Program</h1>
            <hr className="pb-5" />
            <input
              type="text"
